  ...
  return (
    <>
      <div className={`fixed bottom-0 left-0 right-0 z-50 transition-all duration-300 ease-out ${playerExpanded ? 'translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`}>
        <div className="bg-surface-dim text-on-surface px-4 py-3">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={playPrevious} className="p-1.5 hover:bg-surface-container rounded-full transition-colors">
                <SkipBack size={18} />
              </button>
              <button onClick={togglePlay} className="p-2.5 bg-white text-black rounded-full hover:scale-105 transition-transform">
                {isPlaying ? <Pause size={20} /> : <Play size={20} />}
              </button>
              <button onClick={playNext} className="p-1.5 hover:bg-surface-container rounded-full transition-colors">
                <SkipForward size={18} />
              </button>
            ...
            <div className="flex items-center gap-2 w-24">
              <button onClick={() => changeVolume(volume === 0 ? 0.5 : 0)} className="p-1.5 hover:bg-surface-container rounded-full transition-colors">
                {volume === 0 ? <VolumeX size={16} /> : <Volume2 size={16} />}
              </button>
              <input type="range" min="0" max="1" step="0.01" value={volume} onChange={(e) => changeVolume(parseFloat(e.target.value))} className="w-full h-1 bg-surface-container-high rounded-full appearance-none cursor-pointer" />
            </div>

            <button onClick={() => setPlayerExpanded(true)} className="p-1.5 hover:bg-surface-container rounded-full transition-colors">
              <ChevronUp size={16} />
            </button>
          </div>

          <div className="mt-2.5">
            <div ref={progressRef} onClick={handleSeek} className="h-1 bg-surface-container-high rounded-full cursor-pointer relative group">
              <div className="h-full bg-primary rounded-full relative" style={{ width: `${duration > 0 ? (progress / duration) * 100 : 0}%` }}>
